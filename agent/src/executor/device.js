...
class DeviceExecutor {
    ...
    async _setAlarm({ time, hour, minute, duration_minutes, label = 'Alarm' }) {
        // Implementation following the "System Executor" pattern:
        // 1. Structured parameters (hour/minute) preferred.
        // 2. Relative durations (duration_minutes) now supported.

        let h = hour;
        let m = minute;

        // Handle relative timers (e.g., "in 5 minutes")
        if (duration_minutes !== undefined) {
            const target = new Date(Date.now() + duration_minutes * 60000);
            h = target.getHours();
            m = target.getMinutes();
        }
        // If hour/minute aren't provided, try parsing from string
    ...
    async _restartStack() {
        ...
        // We run it in background to avoid killing ourselves if we are part of the stack
    ...
    async _toggleFocus({ enabled }) {
        ...
        try {
            ...
            // Fallback to a simpler notification mute
    ...
    async _toggleHotspot({ enabled }) {
        // Very hard to do via CLI on macOS without private frameworks.
        // We'll open the settings page as a fallback.
    ...
    async _muteNotifications({ enabled }) {
        try {
            ...
            // We don't wait for UI clicking as it's likely to fail without permissions
