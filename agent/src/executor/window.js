...
// ─── WindowExecutor Class ──────────────────────────────────────────────────────
/**
 * WindowExecutor: Manages the native Swift window helper process.
 *
 * Responsibilities:
 *  - Lazy-start helper on first command
 *  - Communicate via stdin/stdout NDJSON
 *  - Timeout per command (2–3s)
 *  - Auto-restart on crash (max 3 consecutive)
 *  - Stop: kill helper, block all further commands until restart()
 *
 * Invariants:
 *  I1: Only invoked AFTER PolicyEngine ALLOW decision
 *  I2: Input is already canonicalized
 *  I3: ContentGuard has already scanned params
 *  I4: Params contain ONLY numbers/enums/strings — no raw user strings beyond app name
 */
class WindowExecutor extends EventEmitter {
    ...
    /**
     * Stop the helper process immediately (panic stop).
     * Rejects all pending requests.
     * Blocks all future commands until restart() is called.
     */
    stop() {
        this._stopped = true;
    ...
    /**
     * Restart the helper after a stop.
     * Resets the stopped flag and restart counter.
     */
    restart() {
        this._stopped = false;
        this._restartCount = 0;
    ...
    /**
     * Check if the helper is running and ready.
     */
    get isReady() {
    ...
    _spawnHelper() {
        return new Promise((resolve, reject) => {
            ...
            // Store startup resolver for the ready message
    ...
    _handleResponse(line) {
        ...
        // Check if this is the startup message
