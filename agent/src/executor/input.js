...
// ─── InputExecutor Class ───────────────────────────────────────────────────────
/**
 * InputExecutor: Manages the native Swift input helper process.
 *
 * Responsibilities:
 *  - Lazy-start helper on first command
 *  - Communicate via stdin/stdout line-delimited JSON
 *  - Timeout per command (move/click/scroll: 2s, drag: 5s)
 *  - Auto-restart on crash (max 3 consecutive)
 *  - Stop button: kill helper, block all further input until restart()
 *
 * Invariants:
 ...
 *  I4: Params contain ONLY numbers/enums — no raw user strings
 */
class InputExecutor extends EventEmitter {
    ...
    _spawnHelper() {
        return new Promise((resolve, reject) => {
            ...
            // Handle stdout (line-delimited JSON responses)
