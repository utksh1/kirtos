/**
 * Unit Tests: Screen Executor (screen.screenshot)
 *
 * Covers:
 *  1. Filename hint sanitization
 *  2. Directory creation
 *  3. Timeout behavior
 *  4. Invalid format rejection (via schema)
 *  5. Path traversal prevention
 *  6. Proper command argument generation
 *  7. Executor does not crash on failure
 *  8. Domain registration validation
 *  9. PolicyEngine integration
 */
...
// ----- Isolated ScreenExecutor class for unit testing -----
// We re-create the core logic here so we can test without spawning processes.
...
// ── Helper: access private methods for testing ──
...
// ──────────────────────────────────────────────
// 6. Domain Registration Validation
// ──────────────────────────────────────────────
...
testAsync('executor times out after configured timeoutMs', async () => {
    // We clear the cache to intercept require('node:child_process')
    ...
    // Mock spawn to return a frozen process that never exits
    ...
    try {
        // use an impossibly fast timeout (e.g. 10ms) to trigger it reliably
...
// Ensure async tests are executed before finishing
